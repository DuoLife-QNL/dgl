...
def retrieve_dataset_info(data: DGLBuiltinDataset) -> Tuple[dgl.DGLGraph, int, int, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    g.ndata['feat'] = g.ndata['feat'].share_memory_()
...
def run(rank, world_size, devices: List[int], histories, dataset_info, conf: DictConfig, prof_utils: Tuple, q: Queue, hydra_output_dir: str):
    ...
    dev_id = devices[rank]
    log.debug("dev_id: {}".format(dev_id))
    ...
    model_args = {
        ...
        'metric': metric,
        'histories': histories # Init cache for each GPU, or one cache for all GPUs. Only works for GAS.
    }
